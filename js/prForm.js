...
function initprForm(JS_OBJECT) {
    var prForm = $('.prForm'),
        ...
        PERSONAL_DATA = prForm.find('input[name=privacy]'),
        ...
        .submit(function (event) {
            ...
            if (submit) $(this).ajaxSubmit({
                ...
                success: function (data) {
                    if (data.ERROR == '') {
                        $(this).clearForm();
                        if(!JS_OBJECT.FIELDS.BASKET) {
                            $.fancybox.close(true);
                            $.fancybox.open($('<div/>', {
                                class: 'success-message modal--lg',
                                html: '<div class="success-message-inner"><div class="modal-title modalForm__title modalForm__group">' + JS_OBJECT.SUCCESS_MESSAGE_TITLE + '</div><div class="modal-description modalForm__group">' + JS_OBJECT.SUCCESS_MESSAGE + '</div><div class="text-center"><a href="#" data-fancybox-close class="adp-btn adp-btn--primary">Вернуться на сайт</a></div></div></div>'
                            }), {padding: 0});
                        }
                        ...
                        if(JS_OBJECT.FIELDS.BASKET){
                            if(JS_OBJECT.FIELDS.BASKET.CODE == 'BASKET'){
                                location.href = SITE_DIR+'order/?ORDER_ID='+data.ORDER_ID;
                            }
                        }
                    }
                    else {
